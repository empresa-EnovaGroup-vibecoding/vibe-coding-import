...
import { type ServiceColumnMapping, SERVICE_FIELD_LABELS } from "./ServicesImportModal";

interface ServiceColumnMappingStepProps {
  headers: string[];
  mapping: ServiceColumnMapping;
  onConfirm: (mapping: ServiceColumnMapping) => void;
  onBack: () => void;
}
...
export default function ServiceColumnMappingStep({
  ...
  onConfirm,
  onBack,
}: ServiceColumnMappingStepProps) {
  const [currentMapping, setCurrentMapping] = useState<ServiceColumnMapping>(mapping);

  const handleFieldChange = (field: keyof ServiceColumnMapping, value: string) => {
  ...
  return (
    <div className="space-y-6">
          ...
          return (
            <div key={field} className="grid grid-cols-2 gap-4 items-center">
              <div className="flex items-center gap-2">
                ...
                {isMapped && <Check className="h-4 w-4 text-green-500" />}
              </div>
              <Select
      ...
      <div className="flex justify-between pt-4">
        ...
        <Button onClick={() => onConfirm(currentMapping)} disabled={!isMappingValid}>
