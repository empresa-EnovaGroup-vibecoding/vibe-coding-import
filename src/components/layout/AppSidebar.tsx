...
export function AppSidebar() {
  ...
  return <>
      ...
      {/* Sidebar */}
      <aside className={cn("fixed left-0 top-0 z-40 h-screen w-64 bg-slate-900/90 dark:bg-slate-950/80 backdrop-blur-2xl border-r border-emerald-900/15 transition-transform duration-300 lg:translate-x-0", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex h-full flex-col">
          {/* Logo / Brand */}
          <div className="flex h-16 items-center gap-3 border-b border-emerald-800/15 px-6">
            {tenant?.logo_url ? (
              <img
                src={tenant.logo_url}
                alt={tenant.name}
          ...
          {filteredNavItems.map(item => {
            ...
            return <NavLink key={item.path} to={item.path} onClick={() => setMobileOpen(false)} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-200", isActive ? "bg-emerald-600/15 backdrop-blur-sm text-emerald-100 shadow-sm" : "text-slate-300 hover:bg-white/10 hover:text-white")}>
          ...
          {/* User section */}
          <div className="border-t border-emerald-800/15 p-3">
            {isSuperAdmin && (
              <NavLink
                to="/super-admin"
                className="flex items-center gap-2 rounded-xl px-3 py-1.5 mb-2 text-[11px] font-medium text-emerald-400/80 hover:bg-white/5 transition-colors"
            ...
            {user && (
              <div className="flex items-center gap-3 rounded-xl bg-white/5 px-3 py-2.5">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-emerald-600/15 text-emerald-400 text-xs font-semibold">
