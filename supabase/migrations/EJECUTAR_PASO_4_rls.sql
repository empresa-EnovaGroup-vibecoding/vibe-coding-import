-- =====================================================
-- PASO 4 DE 5: NUEVAS POLITICAS RLS MULTI-TENANT
...
USING (tenant_id IN (SELECT user_tenant_ids()) OR is_super_admin(auth.uid()));
...
USING (is_owner_of_tenant(tenant_id) OR is_super_admin(auth.uid()));
...
WITH CHECK (is_owner_of_tenant(tenant_id));

CREATE POLICY "Tenant owners can update services"
ON public.services FOR UPDATE TO authenticated
USING (is_owner_of_tenant(tenant_id));
